...
# This constant represents the directions in which a unit can move on 
# the board. We will reference the constant later in the script.
...
# dictionary represents a unit. The key is the position in grid coordinates, 
# while the value is a reference to the unit.
...
func _reinitialize()->void:
	...
	for child in get_children():
		# We can use the "as" keyword to cast the child to a given type. 
		#If the child is not of type Unit, the variable will be null
...
func _flood_fill(cell: Vector2, max_distance: int)-> Array:
	...
	var array := []
	# The way we implemented the flood fill here is by using a stack. 
	# In that stack, we store every cell we want to apply the flood fill algorithm to.
	var stack := [cell]
